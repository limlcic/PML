#!/usr/bin/perl
#
#This file is part of PML-server.
#More than the script processor_single.pl, this script could deal
#with multiple tasks and compress the outputs.
#
...
#Get the boinc app files, prepare for the hard link
opendir DIR,'./';
my @app_files = readdir DIR;
closedir DIR;
@app_files = grep{$_ ne '.' && $_ ne '..' && $_ ne 'tasks.tgz' && $_ !~ /^pml_tar/i}@app_files;

#Create folder for runing and store the outputfiles
mkdir 'outputs';
mkdir 'work';

chdir 'work';

#uncompress the task(s) file
uncompressdir('../tasks.tgz');

#Get files
opendir DIR,'./';
my @task_files = readdir DIR;
closedir DIR;
@task_files = grep{$_ ne '.' && $_ ne '..'}@task_files;
...
#			close FID;
		...
		{
#			rename('out.txt','out.txt.pre');
			copy('out.txt', $out_dir . $task_name);
		}
	}
	else{
...
#			rename('out.txt','out.txt.arff');
			...
			copy('out.txt.arff', $out_dir . $task_name);
		}
	}
	
	if(@ARGV && grep{$_ =~ /debug/}@ARGV){
		
	}
	else{
		#Delete the files in this folder to save the disk space
		opendir(DIR,'./');
...
chdir '../outputs';
opendir DIR_O,'./';
my @outs = readdir DIR_O; 
closedir DIR_O;
@outs = grep{$_ ne '.' && $_ ne '..'}@outs;
my $tar_out = Archive::Tar->new;
$tar_out->add_files(@outs);
$tar_out->write('../outputs.tgz' , 9  );
